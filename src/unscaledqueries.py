...
QUERY_DIR = r"C:\Users\4605DARAMOJ\Desktop\Relational DBMS ML\SQL queries\join-order-benchmark-master\join-order-benchmark-master"
OUTPUT_CSV = r"C:\Users\4605DARAMOJ\Desktop\Relational DBMS ML\SQL queries\job_runs_unscaled.csv"
...
def is_real_job_query(filename, sql_text):
    ...
    return job_file_pattern.match(filename) and sql_text.strip().lower().startswith("select")
...
def main():
    ...
    for f in sorted(os.listdir(QUERY_DIR)):
        if not f.endswith(".sql"):
            continue
        file_path = os.path.join(QUERY_DIR, f)
        with open(file_path, "r", encoding="utf-8") as infile:
            sql_text = infile.read()
        if is_real_job_query(f, sql_text):
            query_files.append(f)

    print(f"Found {len(query_files)} unscaled JOB queries")
